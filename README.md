...
nextlinux-ci: ## Fetch test artifacts for the CI harness
  rm -rf /tmp/test-infra; git clone $(TEST_HARNESS_REPO) /tmp/test-infra
  mv ./nextlinux-ci ./nextlinux-ci-`date +%F-%H-%M-%S`; mv /tmp/test-infra/nextlinux-ci .

.PHONY: clean
clean: nextlinux-ci ## Clean up the project directory and delete dev image
...
The following tasks are provided (run `./nextlinux-ci/ci_harness` to see a current list in case this has changed):
