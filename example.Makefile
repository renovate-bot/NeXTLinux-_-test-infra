############################################################
# Example Makefile for showing usage of the Nextlinux Local
# CI/Test Harness.
############################################################
...
install: deps venv setup.py requirements.txt ## Install to virtual environment
	$(ACTIVATE_VENV) && pip install deps/nextlinux-engine
...
test-cli: setup-test-infra venv ## Run cli-driven end to end tests (assuming cluster is running and set up has been run)
	@$(ACTIVATE_VENV) && $(PYTHON) -m pip install faker && $(PYTHON) nextlinux-ci/cli_driver.py
...
push-dev: setup-test-infra ## Push dev Nextlinux Enterprise Docker image to Docker Hub
	@$(CI_CMD) push-dev-image "$(COMMIT_SHA)" "$(DEV_IMAGE_REPO)" "$(GIT_BRANCH)" "$(TEST_IMAGE_NAME)"

push-rc: setup-test-infra ## (Not available outside of CI) Push RC Nextlinux Enterprise Docker image to Docker Hub
	@$(CI_CMD) push-rc-image "$(COMMIT_SHA)" "$(DEV_IMAGE_REPO)" "$(GIT_TAG)"

push-prod: setup-test-infra ## (Not available outside of CI) Push release Nextlinux Enterprise Docker image to Docker Hub
	@$(CI_CMD) push-prod-image-release "$(DEV_IMAGE_REPO)" "$(GIT_BRANCH)" "$(GIT_TAG)"

push-redhat: setup-test-infra ## (Not available outside of CI) Push prod Nextlinux Enterprise docker image to RedHat Connect
	@$(CI_CMD) push-redhat-image "$(GIT_TAG)"

push-rebuild: setup-test-infra ## (Not available outside of CI) Rebuild and push prod Nextlinux Enterprise docker image to Docker Hub
